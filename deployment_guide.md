...
- Desde **Plataforma > Empresas y licencias** se puede abrir la simulación y elegir un rango de fechas. Si se dejan las fechas vacías, se usa el ciclo pendiente de la empresa; la fecha final seleccionada es inclusiva.
- El simulador utiliza exactamente los costos congelados en los registros de consumo, igual que una liquidación real. El cupo de tokens del plan es un límite operativo y no se resta como una franquicia monetaria gratuita.
- La proyección separa costo del proveedor, fee de Latus, conversión USD/ARS, colchón cambiario, costo estimado de Mercado Pago, impuestos y margen. También informa si el fee, el colchón y la cotización provienen de una regla global o de la empresa.
- Ejecutar una simulación solo realiza lecturas: no crea documentos de liquidación, no registra eventos de cobro y no llama a Mercado Pago ni a proveedores de IA.
- En modo **Piloto**, el administrador debe abrir **Revisar piloto**, comprobar período, consumo, cotización e importe total y confirmar la actualización del próximo cobro. El botón no realiza un débito inmediato: modifica el importe que Mercado Pago cobrará en la renovación.
- La aprobación queda vinculada a una huella del costo, fee, cotización, colchón e importes revisados. Si entra un nuevo consumo o cambia cualquier valor económico antes de aplicar, el backend rechaza la operación y exige revisar una nueva vista previa.
- La ruta general de procesamiento no acepta empresas Piloto, por lo que la revisión no puede omitirse mediante una llamada antigua. La liquidación conserva fecha, usuario, email y huella aprobada para auditoría.
- El control de rentabilidad separa el margen total de la liquidación del margen incremental de IA. Así, el ingreso del plan no puede ocultar un consumo de IA vendido por debajo de su costo real.
- Desde **Plataforma > Cobro automático del consumo de IA** se configuran costo estimado de Mercado Pago, impuestos, margen total mínimo, margen mínimo de IA y la acción global: bloquear o advertir.
- Cada empresa puede heredar esas reglas o definir mínimos y acción propios desde **Administrar licencia**. Las excepciones quedan registradas dentro de la configuración de cobro de esa empresa.
- Antes de llamar a Mercado Pago se descuentan costo del proveedor, comisión de pago e impuestos. Si alguno de los dos márgenes no alcanza el mínimo y la acción efectiva es **Bloquear**, la liquidación queda como `blocked_margin`, no se modifica la suscripción y se genera una alerta crítica.
- En modo **Advertir**, la liquidación puede continuar, pero se genera una alerta operativa. Una alerta abierta se reutiliza para evitar duplicados en cada ejecución del scheduler.
