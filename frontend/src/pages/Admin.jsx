...
export default function Admin() {
  ...
  const canViewUsers = hasPermission(me, "users_view");
  const canUseUsers = hasPermission(me, "users_use");
  const canAdminUsers = hasPermission(me, "users_admin");
  const canViewSettings = hasPermission(me, "settings_view");
  const canAdminSettings = hasPermission(me, "settings_admin");
  const canViewWhatsapp = hasPermission(me, "whatsapp_view");
  const { data: users = [] } = useQuery({ queryKey: ["admin-users"], queryFn: () => api.get("/admin/users").then((r) => r.data), enabled: canViewUsers });
  const { data: settings } = useQuery({ queryKey: ["admin-settings"], queryFn: () => api.get("/admin/settings").then((r) => r.data), enabled: canViewSettings });
    ...
    >
      <div className="p-6 md:p-8 space-y-5 animate-in fade-in duration-300">
        <div className="flex items-center gap-3 bg-[#0B1B26] text-white rounded-sm p-5">
          ...
          <div>
            <p className="font-bold">Panel según tus permisos</p>
            <p className="text-sm text-latus-muted">Las secciones y acciones disponibles se adaptan al nivel asignado en cada módulo.</p>
          </div>
        </div>

        {/* Automatización: Lead sin respuesta */}
        {canViewSettings && <div className="bg-white border border-[#E9E6DC] rounded-sm p-5" data-testid="lead-no-response-settings">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 items-end">
            <div className="flex items-center justify-between sm:flex-col sm:items-start gap-2">
              ...
              <Switch data-testid="lnr-enabled-switch" checked={enabled} disabled={!canAdminSettings} onCheckedChange={setEnabled} className="data-[state=checked]:bg-[#0E8DDB]" />
            </div>
            <div>
              <Label className="text-xs font-semibold">Umbral (horas)</Label>
              <Input data-testid="lnr-threshold-input" type="number" min={1} value={threshold} disabled={!canAdminSettings} onChange={(e) => setThreshold(e.target.value)} className="rounded-sm mt-1" />
            </div>
            <div className="flex items-center justify-between sm:flex-col sm:items-start gap-2">
              <Label className="text-xs font-semibold">Activar horario laboral</Label>
              <Switch data-testid="lnr-business-hours-switch" checked={businessHours} disabled={!canAdminSettings} onCheckedChange={setBusinessHours} className="data-[state=checked]:bg-[#0E8DDB]" />
            </div>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 items-end">
              ...
              <div>
                ...
                <Select value={bhTz} disabled={!canAdminSettings} onValueChange={setBhTz}>
          ...
          <div className="mt-5">
            {canAdminSettings && <Button data-testid="save-settings-button" onClick={() => saveSettings.mutate()} disabled={saveSettings.isPending} className="bg-[#0E8DDB] hover:bg-[#0a7ab8] rounded-sm font-semibold">
        ...
        {canViewWhatsapp && <div className="bg-white border border-[#E9E6DC] rounded-sm p-5" data-testid="whatsapp-integration-card">
          <div className="flex items-center gap-2 mb-1">
        ...
        {canViewUsers && <div className="bg-white border border-[#E9E6DC] rounded-sm overflow-hidden">
              ...
              {users.map((u) => {
                ...
                return (
                  <tr key={u.user_id} data-testid={`user-row-${u.user_id}`} className="hover:bg-latus-cream transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <Select disabled={!canAdminUsers} value={u.role} onValueChange={(v) => update.mutate({ id: u.user_id, body: { role: v } })}>
                    ...
                    <td className="px-5 py-3.5">
                      <Switch
                        ...
                        checked={u.active}
                        disabled={isSelf || !canUseUsers}
                        onCheckedChange={(v) => update.mutate({ id: u.user_id, body: { is_active: v } })}
