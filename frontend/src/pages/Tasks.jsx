...
import { PriorityDot, EmptyState, StatusBadge } from "@/components/Bits";
...
export default function Tasks() {
  ...
  const [form, setForm] = useState({ title: "", description: "", due_date: "", status: "", priority: "medium", assigned_to: "", lead_id: "" });
  ...
  const settingsQ = useQuery({ queryKey: ["settings"], queryFn: () => api.get("/settings").then((r) => r.data) });
  ...
  const openStatuses = taskStatuses.filter((status) => !status.is_done);
  const defaultOpenStatus = openStatuses[0]?.key || taskStatuses[0]?.key || "todo";
  const defaultDoneStatus = taskStatuses.find((status) => status.is_done)?.key || taskStatuses.at(-1)?.key || "done";
  const leadOptions = [
    { value: "none", label: "Sin vincular (General)", searchText: "general sin vincular" },
    ...leads.map((lead) => ({
      value: lead.id,
      label: lead.contact?.name ? `${lead.contact.name} (${lead.title})` : lead.title,
      searchText: `${lead.title} ${lead.contact?.name || ""} ${lead.contact?.company || ""}`,
    })),
  ];
  ...
  const create = useMutation({
    mutationFn: () => api.post("/tasks", {
      ...form,
      status: form.status || defaultOpenStatus,
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      setForm({ title: "", description: "", due_date: "", status: "", priority: "medium", assigned_to: "", lead_id: "" });
  ...
  const filtered = tasks.filter((t) => (tab === "done" ? doneStatuses.has(t.status) : !doneStatuses.has(t.status)));
  const toggleStatus = (task) => doneStatuses.has(task.status) ? defaultOpenStatus : defaultDoneStatus;
  const kanbanColumns = taskStatuses;
              ...
              <div className="space-y-3 py-2">
                ...
                <div>
                  ...
                  <SearchableSelect
                    ...
                    onChange={(v) => setForm({ ...form, lead_id: v === "none" ? "" : v })}
                    options={leadOptions}
                    placeholder="Sin vincular (General)"
                    searchPlaceholder="Buscar contacto o lead..."
              ...
              <div className="space-y-2">
                {filtered.map((t) => (
                  <div key={t.id} data-testid={`task-row-${t.id}`} className="bg-white border border-[#E9E6DC] rounded-sm p-4 flex items-center gap-4 hover:border-zinc-300 transition-colors">
                    ...
                    >
                      {doneStatuses.has(t.status) && <Check className="h-3.5 w-3.5 text-white" />}
                    </button>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className={`font-semibold ${doneStatuses.has(t.status) ? "line-through text-latus-muted" : "text-[#0B1B26]"}`}>{t.title}</p>
                        <StatusBadge list={taskStatusMeta} value={t.status} />
                      </div>
                      ...
                      colTasks.map((t) => (
                        ...
                        >
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={`font-semibold text-sm ${doneStatuses.has(t.status) ? "line-through text-latus-muted" : "text-[#0B1B26]"}`}>{t.title}</p>
